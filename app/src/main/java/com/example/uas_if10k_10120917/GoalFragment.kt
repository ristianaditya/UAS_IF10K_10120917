...
class GoalFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val recyclerview = rootView.findViewById<RecyclerView>(R.id.recyclerviewGoal)
        val list = ArrayList<TopScoredResponse>()
        recyclerview.setHasFixedSize(true)
        recyclerview.layoutManager =
            LinearLayoutManager(requireContext(), LinearLayoutManager.VERTICAL, false)
        val apiService = RetrofitClient.instance

        apiService.getTopScored().enqueue(object : retrofit2.Callback<ArrayList<TopScoredResponse>>  {
            override fun onResponse(
                call: Call<ArrayList<TopScoredResponse>>,
                response: Response<ArrayList<TopScoredResponse>>
            ) {
                if (response.isSuccessful) {
                    response.body()?.let { list.addAll(it) }
                    val adapter = TopScoredAdapter(list)
            ...
            override fun onFailure(call: Call<ArrayList<TopScoredResponse>>, t: Throwable) {
